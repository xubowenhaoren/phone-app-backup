...
public let measuredRSSIAtOneMeterDefault: Double = -57

/// Returns the measured RSSI at 1 meter based on Tx power level extracted from the advertisement data
/// and empirical measurements.
///   - advertisementData: The advertisement data to extract the Tx power level from.
///   - hintIsAndroid: A hint wether the remote device is an Android one.
...
func getMeasuredRSSIAtOneMeter(
    ...
    if txPowerLevel == nil {
        ...
        // From measurements, they seem to advertise at Tx power level 11.
    ...
    // Based on measurements, they mean they are actually transmitting at
    // 20 - abs(txPowerLevel).
    if txPowerLevel < 0 {
    ...
    // (MP1.0), the latter advertising at different power levels.
    switch txPowerLevel! {
        case 12...20:
...
/// Returns the estimated distance in meters based on RSSI, measured RSSI at 1 meter and the
...
///   - environmentalFactor: The environmental factor. Range 2.0-4.0.
