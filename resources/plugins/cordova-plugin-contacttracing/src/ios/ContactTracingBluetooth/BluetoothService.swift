//
//  Created by Zsombor Szabo on 11/03/2020.
//
//

import Foundation

public struct BluetoothService {
    
    /// The string representation of the UUID for the primary peripheral service.
    public static var UUIDPeripheralServiceString = "C019"    
    
    /// The string representation of the UUID for the contact event numbersr characteristic.
    public static var UUIDContactEventNumberCharacteristicString: String = "D61F4F27-3D6B-4B04-9E46-C9D2EA617F62"
}
